...
Merge risk, evidence confidence, merge readiness, file priority, and review actions are separate deterministic outputs. Review actions are prompts for what humans should verify next, not AI commentary or generated fixes. The frontend renders these outputs as a polished developer-tool report with original rabbit branding, compact report navigation, client-side filters, sorting, file details, and evidence/limitations. MergeSignal does not currently assign reviewers, evaluate CODEOWNERS or repository policies, or claim that a pull request is safe or correct.
