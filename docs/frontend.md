...
- observable review-state summary, grammatical concern lifecycle summary, numeric inline-conversation counts, and inline review conversations
- summary metrics
...
The report is derived only from the `POST /api/v1/pull-requests/snapshot` response already returned by the backend. Client-side filters and sorting do not issue additional backend or GitHub requests. CI provider labels render provider and check names separately with centralized display casing so duplicate labels such as `VercelVercel` are avoided. Actionable CI wording is generated by the backend from structured provider/category evidence and reused by Review Briefing, Overview review-next prompts, checklist entries, and Review Actions. Recommended-step and checklist titles are imperative even when a focus item remains descriptive. Review Actions may merge equivalent same-file author-response verification conversations into one visible action while retaining all thread provenance behind expandable details. The file detail drawer shows classification evidence, file context, change magnitude, previous-path classification for renames when available, priority factors, related signal IDs, related review conversations, and limitations; it does not show raw patches. Technical factor IDs remain inside disclosure controls.
...
Regression fixtures cover reviewer-flow reports with a Vercel authorization/configuration blocker, merged same-file author-response verification actions, compact review-summary text, imperative recommended steps, specific top-file review steps, and file-signal traceability for production changes without paired test-file changes. The UI keeps technical provenance hidden by default while preserving expandable detail for deterministic evidence.
